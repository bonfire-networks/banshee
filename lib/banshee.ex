defmodule Banshee do
  ...
  defp select({m, f}, n, d) when is_atom(m) and is_atom(f),
    do: apply(m, f, [n, d])

  defp select({m, a}, n, d) when is_atom(m), do: m.select(a, n, d)

  defp select({m, f, a}, n, d) when is_atom(m) and is_atom(f),
    do: apply(m, f, [a, n, d])
  ...
  defp transform(m, navs, data, xf) when is_atom(m),
    do: m.select(navs, data, xf)

  defp transform({m, f}, n, d, x) when is_atom(m) and is_atom(f),
    do: apply(m, f, [n, d, x])

  defp transform({m, a}, n, d, x) when is_atom(m), do: m.transform(a, n, d, x)

  defp transform({m, f, a}, n, d, x) when is_atom(m) and is_atom(f),
    do: apply(m, f, [a, n, d, x])
